...
class UserReader:
    ...
    def get_users(self, load_threshold: datetime, limit=100) -> List[Dict]:
        # Filter: records updated after the last checkpoint
        filter_criteria = {'update_ts': {'$gt': load_threshold}}

        # Sort by update_ts to ensure correct incremental loading
...
class PgSaver:
    def save_users(self, conn, users: List[Dict]):
        with conn.cursor() as cur:
            for user in users:
                # Convert Mongo ID to string and the rest of document to JSON string
...
class UserLoader:
    ...
    def __init__(self, collection_loader: UserReader, pg_dest_conn, pg_saver: PgSaver, logger):
    ...
    def run_copy(self):
        # 1. Get state (last update_ts)
        wf_setting = self.settings_repository.get_setting(self.pg_dest, self._WORKFLOW_KEY)

        # If no state found, start from very beginning
        if not wf_setting:
            wf_setting = {"last_loaded_ts": datetime.min.isoformat()}

        last_loaded_ts = datetime.fromisoformat(wf_setting["last_loaded_ts"])
        self.log.info(f"Starting load from: {last_loaded_ts}")

        # 2. Load data from Mongo
        users = self.collection_loader.get_users(last_loaded_ts, self._SESSION_LIMIT)
        self.log.info(f"Found {len(users)} documents to load.")

        if not users:
            self.log.info("No new data to load.")
            return

        # 3. Save to PostgreSQL
        self.pg_saver.save_users(self.pg_dest, users)
        self.log.info(f"Saved {len(users)} documents to PostgreSQL.")

        # 4. Update state
        # We take the update_ts of the last document in the sorted list
        new_checkpoint = users[-1]['update_ts']
        ...
        self.log.info(f"Updated checkpoint to: {new_checkpoint}")
...
@dag(
    ...
    catchup=False,
    tags=['sprint5', 'stg', 'users'],
    is_paused_upon_creation=False
)
def stg_load_users_dag():
    ...
    def load_users():
        ...
        # Run Loader
        loader = UserLoader(reader, conn, saver, log)
        loader.run_copy()
...
stg_users_dag = stg_load_users_dag()
